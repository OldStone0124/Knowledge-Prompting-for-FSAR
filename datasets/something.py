...
def make_dataset(root_path, train_list_path, n_samples_for_each_video):
    def build_id2path_dict(regex):
        from glob import glob
        paths = glob(regex)
        dic = {}
        for path in paths:
            dic[os.path.basename(path)] = path
        return dic
    id2path_dict = build_id2path_dict(root_path)

    video_names, labels = load_video_list(train_list_path)
    dataset = []
    count = 0
    final_labels = []
    for i in range(len(video_names)):
        ...
        if '*' not in root_path:
            ...
            video_path = id2path_dict[video_names[i]]
...
class Something(data.Dataset):
    ...
    def __init__(self,
                 ...
                 args=None):
        tobe_hashed = '$'.join(['data', root_path, train_list_path, str(n_samples_for_each_video)]), '$'.join(['class_names', root_path, train_list_path, str(n_samples_for_each_video)])
        hash_object = hashlib.md5(bytes(tobe_hashed[0],'utf-8')), hashlib.md5(bytes(tobe_hashed[1],'utf-8'))
        hashed = hash_object[0].hexdigest(), hash_object[1].hexdigest()
        cache_pths = osp.join('.cache', hashed[0]), osp.join('.cache', hashed[1])
        # set_trace()
        if osp.exists(cache_pths[0]) and osp.exists(cache_pths[1]):
            self.data = pickle.load(open(cache_pths[0],'rb') )
            self.class_names = pickle.load(open(cache_pths[1],'rb') )
            print('load cache success!')
        else:
            self.data, self.class_names = make_dataset(root_path, train_list_path, n_samples_for_each_video)
            pickle.dump(self.data, open(cache_pths[0],'wb') )
            pickle.dump(self.class_names, open(cache_pths[1],'wb') )
