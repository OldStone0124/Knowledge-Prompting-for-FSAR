...
class TemporalShift1D(nn.Module):
    def __init__(self, net, n_segment=3, n_div=8, inplace=False):
        super(TemporalShift1D, self).__init__()
        self.net = net
    ...
    def shift(x, n_segment, fold_div=3, inplace=False):
        ...
        if inplace:
            ...
            out = InplaceShift1D.apply(x, fold)
        else:
...
class InplaceShift1D(torch.autograd.Function):
    ...
    def forward(ctx, input, fold):
        ...
        ctx.fold_ = fold
        n, c, t = x.size()
        buffer = input.data.new(n, fold, t).zero_()
    ...
    if isinstance(net, torchvision.models.ResNet):
        if place == 'block':
            ...
            def make_block_temporal(stage, this_segment):
                ...
                for i, b in enumerate(blocks):
                    # insert TSM for each bottleneck block(or basic block). Its conv1 is transformed.
                    if i % n_round == 0:
